...
async def handle_dim_for_time(dali_dev, address, dim_value, time):
    try:
        time = float(time)
    except ValueError:
        print("Dim_for_time payload must be float!")

    handle_dim_value(dali_dev, address, dim_value)
    asyncio.sleep(time)
    handle_dim_value(dali_dev, address, 0)
                ...
                async for msg in msgs:
                    try:
                        ...
                        if msg.topic.matches("cmd/led/all/dim"):
                            ...
                            await handle_dim_value(
                                dali_dev, GearShort(led_id), dim_value
                            )
                        elif msg.topic.matches("cmd/led/all/dim_for_time"):
                            dim_value, time = msg.payload.decode().split(",")
                            await handle_dim_for_time(
                                dali_dev, Broadcast(), dim_value, time
                            )

                        elif msg.topic.matches("cmd/led/+/dim_for_time"):
                            dim_value, time = msg.payload.decode().split(",")
                            led_id = extract_led_id(str(msg.topic))
                            await handle_dim_for_time(
                                dali_dev, GearShort(led_id), dim_value, time
                            )

                        else:
                            raise Exception("Message with unhandled Topic received!")
